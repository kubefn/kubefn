...
public class RequestDispatcher extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest nettyRequest) {
        ...
        executor.submit(() -> {
            try {
                handleRequest(ctx, method, path, queryParams, headers, body);
            } catch (Exception e) {
    ...
    private void handleRequest(ChannelHandlerContext ctx, String method, String path,
        ...
        if (resolved.isEmpty()) {
            sendError(ctx, 404, "No function registered for " + method + " " + path);
            return;
        }
        ...
        try {
            ...
            sendError(ctx, 503, "Service interrupted");
            return;
        }

        if (!acquired) {
            sendError(ctx, 503, "Group '" + groupName + "' at max concurrency");
            return;
        }
        ...
        try {
            ...
            log.error("Function error: {}.{} for {} {}",
                    groupName, entry.functionName(), method, path, e);
            sendError(ctx, 500, "Function error: " + e.getMessage());
        } finally {
            semaphore.release();
        }
    }
    ...
    private void sendResponse(ChannelHandlerContext ctx, KubeFnResponse fnResponse) {
        try {
            ...
            response.headers().set("X-KubeFn-Runtime", "true");
            ...
            sendError(ctx, 500, "Response serialization error");
        }
    }
